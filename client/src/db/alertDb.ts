...
export const useAlertDB = () => {
    const getSeconds = (frequencyObject: any) => {
        const repeat = frequencyObject.repeat;

        if (frequencyObject.interval === "Minute(s)") {
            return repeat * 60;
        } else if (frequencyObject.interval === "Hour(s)") {
            return repeat * 60 * 60;
        } else if (frequencyObject.interval === "Day(s)") {
    ...
    const getAlertData = async ( loanId: number ) => {
        try {
            const response = await axios.get(`${BACKEND_URL}/alerts?loanId=${loanId}`);
