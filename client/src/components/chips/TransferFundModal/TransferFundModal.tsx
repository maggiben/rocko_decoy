...
const TransferFundModal = ({
    ...
    const eth_balance = ethBalance && Number(ethBalance) > 0.0001 ? Number(ethBalance) - 0.0001 : 0;
        ...
        calls: [
            {
                ...
                data: '0x',
                value: BigInt(Number(ethers.utils.parseEther(eth_balance.toString())))
            },              
            {
    ...
    const { sendUserOperation: batchWithdraw, data } = useContractBatchWrite(config);
    ...
    <ModalContent>
        ...
        <div className="space-y-3">
            <p className="font-medium text-[#141414]">
                You are transferring all funds in your Rocko smart wallet to the address input below
            </p>
        </div>
